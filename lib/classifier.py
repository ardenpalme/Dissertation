...
def save_run(clf, fname, extra=None):
    save_to_pkl({'data': clf.data,
                 'feat_pre_proc': clf.feat_pre_proc,
                 'out_feat_names': clf.out_feat_names,
                 'meta': extra or {}}, fname)

def load_run(clf, fname):
    d = load_from_pkl(fname)
    clf.data = d['data']
    clf.feat_pre_proc = d['feat_pre_proc']
    clf.out_feat_names = d['out_feat_names']
    return d.get('meta', {})
...
class ByzClassifier():
    ...
    def calc_optimal_op_pt(self, in_data=None, cost_fp=1.0, cost_fn=1.0):
        data = in_data if in_data is not None else self.data
        y = data['y_thr']
